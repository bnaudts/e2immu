    ...
    public void test_2() throws IOException {
        ...
        MethodAnalyserVisitor methodAnalyserVisitor = d -> {
            if ("setNegativeJ".equals(d.methodInfo().name)) {
                String expected = switch (d.iteration()) {
                    ...
                    case 1 -> "Precondition[expression=<f*:j><1&&j<1, causes=[escape, escape]]";
                    default -> "Precondition[expression=j<1, causes=[escape, escape]]";
                };
        ...
        TypeAnalyserVisitor typeAnalyserVisitor = d -> {
            if ("EventuallyE1Immutable_2_M".equals(d.typeInfo().simpleName)) {
                String expectApprovedPreconditions = d.iteration() <= 1 ? "{}" : "{j=j<1}";
